...
import type { BloodGlucoseRead } from '@/types/api'

const { data: records, isLoading, isError } = useBloodGlucoseList()
...
function openEdit(record: BloodGlucoseRead) {
...
<template>
  <div>
    <div class="d-flex align-center mb-4 ga-2">
      ...
      <v-spacer />
      <v-btn color="primary" variant="text" prepend-icon="mdi-chart-line" to="/blood-glucose/chart"
        >Chart</v-btn
      >
    ...
    <v-dialog v-model="dialog" max-width="440">
      <v-card>
        <v-card-title>{{ editingId !== null ? 'Edit' : 'Add' }} Blood Glucose</v-card-title>
        <v-card-text>
          <v-row>
            <v-col cols="6">
              <v-text-field
